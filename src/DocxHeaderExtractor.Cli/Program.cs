...
if (options.Command == "a99-canonical-dev-vnext-correctness-preflight" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-live" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-execute" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-prediction-forensic" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-score" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-forensic" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-compatibility-audit" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-binding-repair" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-provenance-approval-audit" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-successor-promotion" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-gold-occurrence-universe-bridge-audit" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-correctness-semantic-contract-alignment-audit" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-true-heading-preflight" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
if (options.Command == "a99-canonical-dev-vnext-true-heading-execute" && options.Inputs.Count == 0)
    options.Inputs.Add("DOC-0116");
// `sample`/`bench`/`eval` có đích mặc định, `info` tự dò mô hình – không cần đầu vào.
if (options.Inputs.Count == 0 && options.Command is not ("sample" or "info" or "bench" or "eval" or "accuracy99" or "r18" or "harness-lift" or "local-qwen-preflight" or "local-qwen-large" or "a99-v2a-role-prefilter-shadow" or "a99-v2a1-role-prefilter-forensic" or "a99-v2a2-role-prefilter-recovery-shadow" or "a99-v6-whole-alias-replay" or "a99-v6-source-slice-audit" or "a99-v6-source-slice-challenger" or "a99-v6-source-fidelity-audit" or "a99-v6-source-fidelity-paired" or "a99-v6-faithful-whole-alias-replay" or "a99-v6-faithful-whole-alias-live" or "a99-v6-faithful-whole-alias-provider-probe" or "a99-semantic-conflict-adjudication" or "a99-hdsa-semantic-node-production-live" or "a99-hdsa-semantic-node-error-decomposition" or "a99-hdsa-semantic-node-production-forensic" or "a99-hdsa-root-v2-live-probe" or "a99-hdsa-semantic-identity-v4-live" or "a99-hdsa-semantic-node-v4-production-live" or "a99-hdsa-v3-v4-parent-forensic" or "a99-semantic-merge-isolation-replay" or "a99-v4-conservative-counterfactual" or "a99-global-outline-llm-live" or "a99-global-outline-llm-evaluate-frozen" or "a99-global-role-identity-live" or "a99-global-role-identity-v2-live" or "a99-global-identity-sparse-positive-replay" or "a99-global-identity-retrieve-verify-live" or "a99-global-identity-oracle-pair-diagnostic" or "a99-global-identity-deterministic-candidate-poc" or "a99-global-identity-deterministic-candidate-poc-evaluate" or "a99-global-identity-deterministic-pair-verification-live" or "a99-canonical-dev-v1" or "a99-canonical-dev-v1-exec-v4" or "a99-canonical-dev-v1-exec-v5" or "a99-canonical-dev-v1-exec-v6" or "a99-canonical-dev-v1-exec-v7-optimized" or "a99-canonical-worker-launch-integrity" or "a99-canonical-worker-lifecycle-self-test" or "a99-provider-hard-timeout-integrity" or "a99-provider-hard-timeout-child" or "a99-canonical-dev-v1-worker" or "a99-doc0116-shape-forensic" or "a99-provider-observability-v1" or "a99-provider-observability-v1-child" or "a99-doc0116-provider-micro-v1" or "a99-doc0116-provider-micro-v1-reclassify" or "a99-provider-timeout-policy-v2"))
{
...
try
{
    return options.Command switch
    {
        ...
        "a99-v2a-role-prefilter-shadow" => await CanonicalDevV2ARolePrefilterShadowRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v2a1-role-prefilter-forensic" => await CanonicalDevV2A1RolePrefilterForensicRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v2a2-role-prefilter-recovery-shadow" => await CanonicalDevV2A2RolePrefilterRecoveryShadowRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        ...
        "a99-v6-source-slice-challenger" => await A99V6SourceSliceChallengerRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v6-source-fidelity-audit" => await A99V6SourceFidelityAuditRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v6-source-fidelity-paired" => await A99V6SourceFidelityPairedRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v6-faithful-whole-alias-replay" => await A99V6FaithfulWholeAliasReplayRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v6-faithful-whole-alias-live" => await A99V6FaithfulWholeAliasLiveRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-v6-faithful-whole-alias-provider-probe" => await A99V6FaithfulWholeAliasProviderProbeRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-semantic-conflict-adjudication" => await SemanticConflictAdjudicationRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-hdsa-semantic-node-production-live" => await HdsaSemanticNodeProductionBenchmarkRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-hdsa-semantic-node-error-decomposition" => await HdsaSemanticNodeProductionErrorDecompositionRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        ...
        "a99-hdsa-v3-v4-parent-forensic" => await HdsaV3V4ParentForensicRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-semantic-merge-isolation-replay" => await HdsaSemanticMergeIsolationReplayRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        ...
        "a99-global-role-identity-v2-live" => await HdsaGlobalRoleIdentityV2LiveRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-sparse-positive-replay" => await HdsaGlobalIdentitySparsePositiveReplayRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-retrieve-verify-live" => await HdsaGlobalIdentityRetrieveVerifyLiveRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-oracle-pair-diagnostic" => await HdsaGlobalIdentityOraclePairDiagnosticRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-deterministic-candidate-poc" => await HdsaDeterministicIdentityCandidateGeneratorPocRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-deterministic-candidate-poc-evaluate" => await HdsaDeterministicIdentityCandidateGeneratorPocRunner.RunFrozenEvaluationAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-global-identity-deterministic-pair-verification-live" => await HdsaDeterministicCandidatePairVerificationLiveRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-v1" => await CanonicalDevV1BaselineRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-v1-exec-v4" => await CanonicalDevV4Runner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-v1-exec-v5" => await CanonicalDevV4Runner.RunV5Async(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-v1-exec-v6" => await CanonicalDevV4Runner.RunV6Async(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-v1-exec-v7-optimized" => await CanonicalDevV7OptimizedRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-preflight" => await CanonicalDevVNextCorrectnessPreflightRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-live" => await CanonicalDevVNextCorrectnessLiveRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-execute" => await CanonicalDevVNextCorrectnessProviderExecutionRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-prediction-forensic" => await CanonicalDevVNextCorrectnessPredictionForensicRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-score" => await CanonicalDevVNextCorrectnessGoldScoringRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-forensic" => await CanonicalDevVNextCorrectnessGoldForensicRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-compatibility-audit" => await CanonicalDevVNextCorrectnessGoldCompatibilityAuditRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-binding-repair" => await CanonicalDevVNextCorrectnessGoldBindingRepairRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-provenance-approval-audit" => await CanonicalDevVNextCorrectnessGoldProvenanceApprovalAuditRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-successor-promotion" => await CanonicalDevVNextCorrectnessGoldSuccessorPromotionRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-gold-occurrence-universe-bridge-audit" => await CanonicalDevVNextCorrectnessGoldOccurrenceUniverseBridgeAuditRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-correctness-semantic-contract-alignment-audit" => await CanonicalDevVNextCorrectnessSemanticContractAlignmentAuditRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-true-heading-preflight" => await CanonicalDevVNextTrueHeadingPreflightRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-dev-vnext-true-heading-execute" => await CanonicalDevVNextTrueHeadingProviderExecutionRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-worker-lifecycle-self-test" => await CanonicalWorkerLaunchIntegrityRunner.RunLifecycleSelfTestAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-canonical-worker-launch-integrity" => await CanonicalWorkerLaunchIntegrityRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-provider-hard-timeout-integrity" => await ProviderHardTimeoutIntegrity.RunSelfTestAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-provider-hard-timeout-child" => ProviderHardTimeoutIntegrity.RunChildMode(),
        "a99-canonical-dev-v1-worker" => await CanonicalDevV1BaselineRunner.RunWorkerAsync(cts.Token),
        "a99-doc0116-shape-forensic" => await CanonicalDevDoc0116ShapeForensicRunner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-provider-observability-v1" => await ProviderObservabilityV1Runner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-provider-observability-v1-child" => ProviderObservabilityV1Runner.RunChildMode(),
        "a99-doc0116-provider-micro-v1" => await Doc0116ProviderMicroV1Runner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-doc0116-provider-micro-v1-reclassify" => await Doc0116ProviderMicroV1Runner.RunOfflineReclassificationAsync(Directory.GetCurrentDirectory(), cts.Token),
        "a99-provider-timeout-policy-v2" => await ProviderTimeoutPolicyV2Runner.RunAsync(Directory.GetCurrentDirectory(), cts.Token),
...
static int RunBench(CommandLineOptions o)
{
    ...
    foreach (var doc in DocxHeaderExtractor.Eval.BenchDocumentFactory.All())
    {
        var path = DocxHeaderExtractor.Eval.BenchDocumentFactory.Write(doc, dir);
...
static async Task<int> RunReviewAsync(CommandLineOptions o, CancellationToken ct)
{
    ...
    var source = new AuthorityDocumentSourceReader(o.Pipeline.Extraction).Read(file).Document;
    var agentRun = await harness.RunAsync(AgentRequest(file, o), ct);
    var outline = agentRun.TaskResult.Value;
...
static Task<int> RunPdfTagsAsync(CommandLineOptions o, CancellationToken ct)
{
    ...
    var reports = new List<PdfTaggedHeadingAnalyzerReport>();
    foreach (var input in inputs)
    {
        ...
        {
            reports.Add(new PdfTaggedHeadingAnalyzerReport(input, 0, 0, 0, 0, "no-sibling-pdf", TaggedStructureTrace.Empty, []));
            continue;
        }
        var report = PdfTaggedHeadingAnalyzer.Analyze(pdf,
...
static async Task<int> RunKeyRebaseAsync(CommandLineOptions o, CancellationToken ct)
{
    ...
    var source = new AuthorityDocumentSourceReader(o.Pipeline.Extraction).Read(documentPath).Document;
