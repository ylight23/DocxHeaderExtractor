...
// Accuracy-99 document-first review is an evaluation-only lane. It reads parser-owned DOCX
// paragraphs directly and never calls the production authority pipeline or a provider.
    ...
    CancellationToken ct) =>
{
    ...
    try
    {
        ...
        req.HttpContext.Response.Headers["X-DHX-Review-Status"] = session.Frozen ? "GOLD_FROZEN" : ManifestStatus(session.Packet.Manifest);
...
app.MapGet("/api/accuracy99/review/session/{id}", (string id, Accuracy99DocumentFirstReview reviews, HttpResponse res) =>
{
    try
    {
        var session = reviews.Load(id);
        res.Headers["X-DHX-Review-Status"] = session.Frozen ? "GOLD_FROZEN" : ManifestStatus(session.Packet.Manifest);
        return Results.Text(Accuracy99DocumentFirstReview.ToJsonLines(session.Packet), "application/x-ndjson", Encoding.UTF8);
    }
...
app.MapPut("/api/accuracy99/review/session/{id}", async (string id, HttpRequest req, Accuracy99DocumentFirstReview reviews, CancellationToken ct) =>
{
...
app.MapPost("/api/accuracy99/review/session/{id}/finalize", async (string id, Accuracy99DocumentFirstReview reviews, CancellationToken ct) =>
{
    try
    {
...
app.MapPost("/api/accuracy99/review/session/{id}/compare", async (string id, Accuracy99DocumentFirstReview reviews, CancellationToken ct) =>
{
    try
    {
