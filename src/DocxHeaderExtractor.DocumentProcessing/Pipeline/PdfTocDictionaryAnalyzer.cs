...
internal sealed record PdfTocDictionaryAnalyzerResult(
    int TocPage,
    int Entries,
...
{
    public static PdfTocDictionaryAnalyzerResult Empty { get; } = new(0, 0, 0, 0, 0, []);
}
...
/// </summary>
internal static class PdfTocDictionaryAnalyzer
{
    ...
    public static PdfTocDictionaryAnalyzerResult Analyze(IReadOnlyList<PdfLine> lines)
    {
        if (lines.Count == 0) return PdfTocDictionaryAnalyzerResult.Empty;
        ...
        if (toc.Entries is not { Count: >= 5 }) return PdfTocDictionaryAnalyzerResult.Empty;
        ...
        return new PdfTocDictionaryAnalyzerResult(
            toc.Page,
            items.Length,
