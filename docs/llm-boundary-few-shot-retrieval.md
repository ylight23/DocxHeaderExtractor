...
|---|---|---|
| Lưu trữ correction đã người xác nhận | [`CorrectionMemory`](../src/DocxHeaderExtractor.Core/Learning/CorrectionMemory.cs) — JSONL append-only, `VerifiedCorrection` record, `SaveChangedAsync`/`Load` | Record hiện tại là `PredictedLevel`/`CorrectedLevel` (sửa CẤP) — cần thêm biến thể cho sửa RANH GIỚI (`OriginalText`, `HeadingSpan`), tái dùng type có sẵn (`HeadingRecord.HeadingSpan`/`InlineBodySpan`/`InlineBody` đã đúng hình dạng cần) |
