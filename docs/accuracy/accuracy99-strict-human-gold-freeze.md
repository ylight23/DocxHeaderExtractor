...
The strict DEV cohort is frozen at 15 documents using DEV metadata only. `DOC-0255` and `DOC-0259` remain valid schema references, but are classified as `HUMAN_REVIEWED_MODEL_ASSISTED` and are excluded from the strict A99 claim. They are not promoted or renamed.

Strict authority state:

- Strict cohort: `15` documents.
- Strict Human Gold valid and eligible: `0/15`.
- Assisted pilot schema-valid: `2/2` (`DOC-0255`, `DOC-0259`).
- Holdout: sealed and untouched.
- Baseline: not run.

The next strict review is source-only `DOC-0202`:

`C:\A99-Gold\packets\dev\DOC-0202.v1.json`
...
- Focused A99 tests: `40/40` passed.
- Release build: passed.
- Full suite: `1114` total, `1113` passed, `1` frozen `N15` failure, `0` skipped.
- New failures: `0`.
- Provider calls: `0`.
