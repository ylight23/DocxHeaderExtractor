...
Verification: focused checkpoint/lane tests passed `22/22`; Core, CLI, Web, and test projects built successfully without a RID. A `win-x64 --no-restore` build was not available because the existing assets files do not contain `net9.0/win-x64` targets. No provider calls were made.
