...
public sealed class SourceAuthorityCutoverTests
{
    ...
    public async Task Docx_route_materializes_aggregate_batch_telemetry()
    {
        ...
        try
        {
            ...
            Assert.Equal(1, telemetry.TotalProviderCalls);
            Assert.Equal(1, telemetry.TotalResponses);
            Assert.Equal(1, audit.ModelRequests.Count(request => request.ProviderCallAttempted));
            Assert.Equal(1, audit.RawAnalystResponses.Count);
        }
        finally
        {
            File.Delete(sourcePath);
