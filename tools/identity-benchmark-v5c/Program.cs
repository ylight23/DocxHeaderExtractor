...
internal static class Program
{
    ...
    private const string OutputRelative = "artifacts/identity-benchmark/v5/semantic-node-induction/evaluation";
    ...
    private static Evaluation Evaluate(object frozenObject, JsonElement gold, string root)
    {
        ...
        foreach (var g in gold.GetProperty("responses").EnumerateArray())
        {
            ...
            if (status == "VALID")
            {
                ...
                Require(hasLeft && hasRight, $"V5C_VALID_CLUSTER_MISSING_ASSIGNMENT:{candidateId}");
                if (leftNode.GetString() != rightNode.GetString()) predicted = "DISTINCT";
                else if (HasEdge(p.GetProperty("continuationEdges"), left, right)) predicted = "CONTINUATION_OF";
                else predicted = "SAME_SEMANTIC_REPEAT";
                ownership = predicted == goldLabel ? "CORRECT" : FailureOwnership(goldLabel, predicted);
            }
            rows.Add(new Row(g.GetProperty("reviewId").GetString()!, candidateId, g.GetProperty("leftOccurrenceId").GetString()!, g.GetProperty("rightOccurrenceId").GetString()!, goldLabel, predicted, ownership, status));
        }
    ...
    private static Evaluation BuildEvaluation(IReadOnlyList<Row> rows, string root)
    {
        ...
        var correct = rows.Count(x => x.Predicted == x.Gold);
        ...
        {
            var tp = rows.Count(x => x.Gold == label && x.Predicted == label);
            var fp = rows.Count(x => x.Gold != label && x.Predicted == label);
            var fn = rows.Count(x => x.Gold == label && x.Predicted != label);
        ...
        var summary = new
        {
            schemaVersion = "a99-v5c-semantic-node-evaluation-summary-v1",
            phase = "V5C_GRAPH_TO_PAIR_EVALUATION",
            ...
            falseMerge, falseSplit, perRelation,
        ...
        var perItem = rows.Select(x => new { reviewId = x.ReviewId, candidateId = x.CandidateId, leftOccurrenceId = x.Left, rightOccurrenceId = x.Right, goldRelation = x.Gold, predictedRelation = x.Predicted, predictionStatus = x.Status, predictionValid = x.Predicted != "INVALID_PREDICTION", failureOwnership = x.Ownership, correct = x.Predicted == x.Gold }).ToArray();
        var manifest = new { schemaVersion = "a99-v5c-evaluation-manifest-v1", status = "V5C_COMPLETE", goldReadAfterPredictionFreeze = true, predictionFreezeStatus = "V5C_PREDICTIONS_FROZEN_BEFORE_GOLD", itemCount = rows.Count, exactCandidateJoin = rows.Count, modelCalls = 0, providerCalls = 0, pairLabelsDerived = true, source = "FROZEN_V5B_PRIMARY_ATTEMPTS" };
    ...
    private static bool HasEdge(JsonElement edges, string left, string right) => edges.EnumerateArray().Any(x => x.GetProperty("fromOccurrenceId").GetString() == left && x.GetProperty("toOccurrenceId").GetString() == right);
    private static string FailureOwnership(string gold, string predicted) => gold == "DISTINCT" && predicted is "SAME_SEMANTIC_REPEAT" or "CONTINUATION_OF" ? "WRONG_NODE_MERGE" : gold is "SAME_SEMANTIC_REPEAT" or "CONTINUATION_OF" && predicted == "DISTINCT" ? "WRONG_NODE_SPLIT" : "WRONG_CONTINUATION_EDGE";
    ...
    private sealed record Row(string ReviewId, string CandidateId, string Left, string Right, string Gold, string Predicted, string Ownership, string Status);
